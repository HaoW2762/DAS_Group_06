...
The distribution of the variables **Year** and **Budget** of the movies in the two groups, rates greater than 7 and less than 7, are symmetric and the distribution of the **Length** of the films in the two groups has a significant difference, which can be seen the length of the films with the rate less than 7 is more likely longer than those with the rate greater than 7 and has a minor variance.



- Films with rating greater than 7 are higher in more modern publishing years

- Films with rating greater than 7 are shorter in broadcast time

- Films with rating greater than 7 are higher have higher budget
...
```{r data2,echo = TRUE, echo = FALSE, eval = TRUE, warning = FALSE, message = FALSE}

#glm commend used to fit model 
...
After we obtain a 95% confidence interval for the odds by simply exponentiatin the lower and upper bounds of our log-odds interval and generate a plot as following:
...
From the plot, the confidence interval gives a range of plausible values for variables' coefficients.
...
```{r, echo = TRUE,eval = TRUE,  out.width = '80%', fig.align = "center", warning = FALSE,  fig.pos="H",fig.cap = '\\label{fig:rate} Comparing predictions and observations.'}
